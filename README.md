...
Polling intentionally retrieves the newest 100 messages in the bot's combined feed without a sender or channel narrow, so recent denied traffic remains auditable. Zulip does not include channel history that the bot account cannot access, including channels it has not joined; subscribe the bot to every channel whose rejected traffic must be audited. Sender, route, and fail-closed policy rejections are logged before durable retirement, then discarded before session routing, reactions, slash-command handling, or Hermes invocation. Later admission rejections, such as a missing mention or steering from a different authorized sender, are also logged and cannot reach Hermes. Audit delivery is at least once: normal polls deduplicate by message ID, while a crash or failed state write after logging can produce a duplicate on retry. Audit events contain only a stable reason and numeric Zulip message ID; sender identity, message text, stream/topic names, and credentials are never included.

The bridge writes audit events to standard output. Production deployments must send standard output to a size-bounded, rotating log sink; the package cannot rotate a parent process manager's output stream. The 100-message polling window is intended for small, continuously running deployments. Use Zulip event queues or a cursor-based collector when traffic can exceed that window between successful polls. See the [Zulip message retrieval documentation](https://zulip.com/api/get-messages) for combined-feed visibility and pagination semantics.
...
The bridge fences chat, attachment, route, history, and steering text as untrusted prompt data. Topic history includes only allowlisted human senders and excludes all bot-authored replies. Every config must declare package-supported `hermes.toolsets`; unrestricted aggregate/platform toolsets, free-form `hermes.extra_args`, abbreviated flag overrides, and `--yolo` are rejected again at runtime. Fencing and toolsets are still not an OS sandbox, so use a dedicated Hermes profile, OS account, container, or equivalent execution boundary when chat-originated work must not inherit the bridge account's filesystem, process, or network access.
...
Version `0.2.0` is a breaking security release. Before restarting an existing service, add the sender, stream ID, topic, mention, slash-command, restricted Hermes toolset, and notifier policies shown above and run `validate-config`. Legacy notifier tasks that carry callback targets only in free-text bodies are intentionally ignored; recreate them with structured `metadata.notification_target` or `source_detail.notification_target` fields.
